...
async def run_agent(req: RunRequest):
    ...
    Also resets chat_history to [] at the start of every run — a rerun for
    the same rep produces a brand new pipeline output, so any prior chat
    turns (which may reference stale deal data) must not be replayed
    alongside it. See _fix for the chat-history/session-reuse gap.
    """
    session = await session_service.get_session(
        app_name="sales_rep_pipeline",
        user_id=req.user_id,
    ...
    if session.state.get("chat_history"):
        await session_service.append_event(
            session,
            Event(
                author="system",
                actions=EventActions(state_delta={"chat_history": []}),
            ),
        )
