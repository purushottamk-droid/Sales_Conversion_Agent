...
def _markdown_lite_to_html(text: str) -> str:
    """Turns the LLM's plain-text summary (which the prompt instructs to
    mark account names / opportunity names / key figures with **bold**,
    and list items with a leading '- ') into readable HTML.

    HTML-escapes first so account/deal names containing '<', '>', '&'
    (real Salesforce/Gong data, not under our control) can't break the
    markup — markdown syntax is applied afterward on the escaped text,
    since '**' and '- ' aren't affected by escaping.
    """
    if not text:
        return ""
    ...
    def _flush_list():
        if current_list:
            ...
            paragraphs_html.append(f"<ul style='margin:4px 0 12px 0; padding-left:20px;'>{items}</ul>")
            current_list.clear()
    ...
    for para in escaped.split("\n\n"):
        ...
        for line in lines:
            ...
            if stripped.startswith("- ") or stripped.startswith("* "):
                current_list.append(_BOLD_RE.sub(r"<b>\1</b>", stripped[2:].strip()))
            else:
                _flush_list()
                if stripped:
                    line_html = _BOLD_RE.sub(r"<b>\1</b>", stripped)
                    paragraphs_html.append(f"<p style='margin:0 0 10px 0;'>{line_html}</p>")
        _flush_list()
...
async def notify_manager(
    ...
    body_html = f"""
    <html><body style="font-family: Arial, sans-serif; line-height: 1.6; color: #2c3e50; max-width: 640px;">
    <h2 style="color: #c0392b; margin-bottom: 4px;">Rep Performance Briefing: {html.escape(rep_name)}</h2>
    <p style="color: #7f8c8d; margin-top: 0;">Rep ID: {html.escape(rep_id)}</p>

    <h3 style="border-bottom: 1px solid #ecf0f1; padding-bottom: 4px;">Performance Summary</h3>
    {_markdown_lite_to_html(performance_summary)}

    <h3 style="border-bottom: 1px solid #ecf0f1; padding-bottom: 4px;">Recommended Actions</h3>
    {_markdown_lite_to_html(recommended_actions)}
...
async def message_rep(
    ...
    <div style="background: #f9f9f9; border-left: 4px solid #2980b9; padding: 12px 16px; margin: 16px 0;">
      {_markdown_lite_to_html(findings_summary)}
    </div>
