...
//   1. createSession
//   2. runPipeline (SSE) -> updates nodeStates/nodeDetails/activeSources/keysShown live
//   3. getResult -> populates `result`, flips dashboardVisible on
//
// Node 1 (DataCollectionAgent)'s detail text is now computed from the real
...
import { createSession, getResult, runPipeline } from '../api/pipelineClient';
  ...
  const run = useCallback(
    async (config = {}) => {
      ...
      try {
        ...
        const finalResult = await getResult(sessionId, userId);
